...
function FundFlowDiagram() {
  const nodes = [
    { id: "user", label: "User", x: 50, y: 60, color: "#00F5FF" },
    { id: "router", label: "Router", x: 200, y: 60, color: "#00F5FF" },
    { id: "poolA", label: "Pool A", x: 350, y: 20, color: "#C026D3" },
    { id: "poolB", label: "Pool B", x: 350, y: 100, color: "#C026D3" },
    { id: "treasury", label: "Treasury", x: 500, y: 20, color: "#F59E0B" },
    { id: "lp", label: "LP Holders", x: 500, y: 100, color: "#22C55E" },
  ]
      ...
      {edges.map((edge, i) => {
        ...
        return (
          <g key={i}>
            ...
            />
            <text x={midX} y={midY - 6} textAnchor="middle" fill="#64748B" fontSize="8" fontFamily="monospace">
              {edge.label}
            </text>
          </g>
        )
      ...
      {nodes.map((node, i) => (
        ...
        >
          <rect
            ...
            opacity="0.8"
          />
          <text x={node.x} y={node.y + 4} textAnchor="middle" fill={node.color} fontSize="9" fontFamily="monospace">
            {node.label}
          </text>
        </motion.g>
      ))}
...
export default function BlockchainSecurityPage() {
  const [mode, setMode] = useState<"web" | "blockchain">("blockchain")
  ...
  const handleScan = () => {
    toast.success("Contract scan initiated!", {
      description: "Analyzing bytecode and source code...",
    })
    setScanned(true)
  }
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Audit smart contracts for vulnerabilities and gas optimizations.
          </p>
        </div>
        <div className="flex items-center rounded-lg border border-border bg-secondary p-0.5">
          <Button
            variant="ghost"
            size="sm"
            className={`gap-1.5 rounded-md ${mode === "web" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"}`}
            onClick={() => setMode("web")}
          >
          ...
          <Button
            variant="ghost"
            size="sm"
            className={`gap-1.5 rounded-md ${mode === "blockchain" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"}`}
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            variant="outline"
                            size="sm"
                            className="text-xs border-border text-muted-foreground hover:text-foreground hover:bg-secondary font-mono"
                            onClick={() => {
            ...
            >
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                <Card className="bg-card border-border">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-foreground text-base">Risk Assessment</CardTitle>
                  </CardHeader>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                      {blockchainVulns.map((vuln) => (
                        <div
                          key={vuln.type}
                          className={`rounded-lg border p-3 ${severityBg[vuln.severity]}`}
                        >
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-xs font-medium">{vuln.type}</span>
                            <Badge variant="outline" className={`text-[10px] ${severityBg[vuln.severity]}`}>
                              {vuln.count}
                            </Badge>
                          </div>
              ...
              <Tabs defaultValue="gas" className="w-full">
                <TabsList className="bg-secondary border border-border">
                  <TabsTrigger value="gas" className="data-[state=active]:bg-card data-[state=active]:text-foreground text-muted-foreground">
                    <Fuel className="h-3.5 w-3.5 mr-1.5" />
                    Gas Optimization
                  </TabsTrigger>
                  <TabsTrigger value="fixes" className="data-[state=active]:bg-card data-[state=active]:text-foreground text-muted-foreground">
                          ...
                          {gasOptimizations.map((opt) => (
                            <TableRow key={opt.function} className="border-border hover:bg-secondary/50">
                              ...
                              <TableCell className="text-muted-foreground font-mono text-sm">
                                {opt.currentGas.toLocaleString()}
                              </TableCell>
                              <TableCell className="text-success font-mono text-sm">
                                {opt.optimizedGas.toLocaleString()}
                              </TableCell>
                              <TableCell>
                          ...
                          .map((vuln) => (
                            <div key={vuln.type} className="flex items-start gap-3 p-3 rounded-lg border border-border bg-secondary/30">
                              ...
                              <Button
                                size="sm"
                                variant="outline"
                                className="gap-1 border-border text-foreground hover:bg-secondary text-xs shrink-0"
                                onClick={() => toast.success(`AI fix generated for ${vuln.type}`)}
                              >
      ...
      {mode === "web" && (
        <Card className="bg-card border-border">
          <CardContent className="p-12 text-center">
            ...
            <h2 className="text-lg font-semibold text-foreground mb-2">Web Security Mode</h2>
            <p className="text-sm text-muted-foreground max-w-md mx-auto mb-4">
              Switch to Blockchain Mode for smart contract auditing, or go to Repositories to scan web applications.
            </p>
